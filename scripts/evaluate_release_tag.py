...
class DocBuildDecider:
    def __init__(self, git: GitReader):
        self.git = git

    def should_build_docs(self, tag: Version) -> bool:
        existing_tags = self.git.get_existing_tag_versions()
        ...
        return should_build_docs


def main(args: argparse.Namespace) -> None:
    tag = Version(args.tag)
    builder = DocBuildDecider(GitReader())
    if not builder.should_build_docs(tag):
